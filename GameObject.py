...
class Character(GameObject):
    def __init__(self, speed, health, x, y,image_path,screen):
        ...
        self.standing = pygame.image.load("bobby.png")
        self.inAir = False
    ...
    def playerMovementControl(self,event):
        ...
        if all(key == 0 for key in pygame.key.get_pressed()):
            self.screen.blit(self.standing,tuple(self.currentPosition),(0,0,100,76) )
        elif event[pygame.K_RIGHT]:
            self.screen.blit(self.walkingR,tuple(self.currentPosition),(101*self.nexImage,0,100,76))
            self.currentPosition[0]+=self.speed
            self.nexImage +=1
            if(self.nexImage==3):
                self.nexImage=0
        elif event[pygame.K_LEFT]: 
            self.screen.blit(self.walkingL,tuple(self.currentPosition),(101*self.nexImage,0,95,76))
            self.currentPosition[0]-=self.speed
            self.nexImage +=1
            if(self.nexImage==3):
                self.nexImage=0
